...
public class GameOver extends Fragment {
    ...
    private NavController navScore;

    @Override
    public void onViewCreated(@NonNull @NotNull View view, @Nullable @org.jetbrains.annotations.Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);

        //bundle to get the bundle info sent from the previous fragment
        bundle=getArguments();
        tv = view.findViewById(R.id.txtViewRetry);

        if((int)bundle.get("Score") <=3) {
            tv.setText("You scored " + bundle.get("Score") + " out of 5" +
                    ". Try again :(");
        }else{
            tv.setText("You scored " + bundle.get("Score") + " out of 5" +
                    ". You are a STAR");
        ...
        btnOver = view.findViewById(R.id.btnRetry);
        btnOver.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                navScore = Navigation.findNavController(view);
                navScore.navigate(R.id.action_gameOver_to_questionsView);
            }
        });
    }
}